...
class InverseLadder(Benchmark):
    """
    Represents the inverse of CNOT ladder benchmark parameterized by the number of qubits n (> 1).
    ...
    def reference_circuits(self) :
        ...
        for idx in range(self.n-1) :
            clifford_repr.cx(rep_data[idx], rep_data[idx+1])
    ...
    def dynamic_circuit(self, mcm=True, stretch_dd=False) :
        ...
        for i in range(self.n) : qc.h(data[i])

        for i in range(self.n-1, 0, -1) : qc.cx(data[i], anc[i-1])
        for i in range(self.n-2, -1, -1) : qc.cx(anc[i], data[i])
        ...
        parity = expr.lift(c1[0])
        for i in range(1, self.n) :
            with qc.if_test(parity) :
                qc.z(data[i])
            if i < self.n - 1 :
                parity = expr.bit_xor(c1[i], parity)
    ...
    def guppy_circuits(self) :
        raise NotImplementedError('Inverse ladder not available for Guppy')
